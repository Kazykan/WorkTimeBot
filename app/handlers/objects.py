...
async def objects_active_only_callback(callback: types.CallbackQuery, state: FSMContext):
...
@router.callback_query(lambda c: c.data.startswith("object_"))
async def object_details_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with db_session() as session:
        ...
        await callback.message.edit_text(
            info_text,
            reply_markup=get_object_actions_keyboard(work_object, total_hours, total_payments),
            parse_mode="HTML"
        )
...
async def complete_object_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with db_session() as session:
        ...
        work_object = await object_repo.update_status(object_id, user.id, ObjectStatus.COMPLETED)
        if work_object:
...
async def reopen_object_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with db_session() as session:
        ...
        work_object = await object_repo.update_status(object_id, user.id, ObjectStatus.ACTIVE)
        if work_object:
